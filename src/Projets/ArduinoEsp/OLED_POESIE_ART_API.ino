...
const DISCORD_SYNC_URL = "https://rescoe.xyz/api/discord/sync-to-discord";

async function triggerDiscordSync() {
  try {
    const r = await fetch(DISCORD_SYNC_URL, {
      method: "POST"
    });

    if (!r.ok) {
      throw new Error(`Sync Discord HTTP ${r.status}`);
    }

    return r.json().catch(() => null);
  } catch (err) {
    console.error("triggerDiscordSync error", err);
    setStatus(`Sync Discord échouée: ${err?.message || err}`);
    return null;
  }
}
