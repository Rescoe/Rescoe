...
const RESCOE_SYNC_URL = 'https://www.rescoe.xyz/api/esp/sync-to-discord';
...
// REMPLACER notifyRescoeDiscord par cette version avec retry
async function notifyRescoeDiscord(payload) {

    try {
    if (!payload || typeof payload !== 'object') payload = {};

    // Compacter oledBuffer simple (still/poetry)
    if (payload.oledBuffer && Array.isArray(payload.oledBuffer)) {
      payload.oledBufferCompact = payload.oledBuffer.map(b =>
        (b & 0xFF).toString(16).padStart(2, '0')
      );
      delete payload.oledBuffer;
    }

    // Compacter toutes les frames (gif/anim) — format [{hex, delay}, ...]
    if (Array.isArray(payload.frames) && payload.frames.length) {
      payload.framesCompact = payload.frames.map(fr => ({
        // buffer as hex string concaténé (1024 bytes = 2048 chars)
    ...
    if (!payload.oledBufferCompact) payload.oledBufferCompact = [];


    const body = JSON.stringify(payload);
    console.log('[ESP relay] size:', body.length);
    console.log('[ESP relay] meta:', {
      type: payload.type,
      mode: payload.mode,
      hasCompact: !!payload.oledBufferCompact,
      compactLen: payload.oledBufferCompact.length
    });

    // Retry jusqu'à 3 fois avec délai croissant (ESP peut être occupé)
    const MAX_RETRIES = 3;
    let lastError = null;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        if (attempt > 1) {
          ...
          console.log(`[ESP relay] retry ${attempt}/${MAX_RETRIES} dans ${waitMs}ms...`);
        ...
        const r = await fetch('/notify-discord', {
          method: 'POST',
        ...
        console.log('[ESP relay] status:', r.status);

        if (!r.ok) {
          console.warn('[ESP relay] HTTP ERROR', r.status, txt);
          lastError = 'HTTP ' + r.status;
          // Si c'est un 400 (JSON invalide côté ESP), pas la peine de retry
        ...
        setStatus('Discord OK ✓');
        return true;

      } catch (fetchErr) {
        lastError = fetchErr?.message || String(fetchErr);
        console.warn(`[ESP relay] attempt ${attempt} exception:`, lastError);
        // TypeError: Failed to fetch = ESP occupé, on retry
      }
    }
    ...
    console.warn('[ESP relay] fatal exception', e);
...
await new Promise(resolve => setTimeout(resolve, 1500));
      ...
      const poetryFrames = sendContext.poetryFrames || frames;
...
await notifyRescoeDiscord({
  ...
  oledBufferCompact: firstFrame
    ? Array.from(firstFrame.buffer, b =>
        (b & 0xFF).toString(16).padStart(2, '0')
      )
    : []
});
...
await fetch(
  `/username?n=${encodeURIComponent(name)}&ts=${encodeURIComponent(timestamp)}&a=${encodeURIComponent(artistName)}`,
  { cache: 'no-store' }
);
...
// AJOUTER CE DÉLAI avant notifyRescoeDiscord
await new Promise(r => setTimeout(r, 800)); // laisser l'ESP se libérer
...
await notifyRescoeDiscord({
  ...
  ethAddress: '',
  text: sendContext.poetryText || '',
  oledBufferCompact: Array.from(buf, b =>
    (b & 0xFF).toString(16).padStart(2, '0')
  )
});
...
const firstFrame =
  (frames && frames.length) ? frames[0] : null;

// ── BLOC ŒUVRE ANIMÉE — remplacer le notifyRescoeDiscord par : ──
await notifyRescoeDiscord({
  ...
  // Toutes les frames, pas juste la première
  frames: frames.map(fr => ({
    buffer: Array.from(fr.buffer),
    delay: fr.delay
  }))
  // PAS d'oledBufferCompact ici — on envoie frames directement
});
    ...
    // 🔧 PUSH IMAGE
    await fetch('/draw', { method: 'POST', body: pendingBuf });

    // 🔧 CONVERSION BUFFER → HEX COMPACT
    const oledBufferCompact = Array.from(pendingBuf, b =>
      (b & 0xFF).toString(16).padStart(2, '0')
    );

    // 🔧 DISCORD PAYLOAD (ALIGNÉ AVEC LES AUTRES)
    await notifyRescoeDiscord({
      uid: makeClientUid({
        type: 'still',
        mode: 'still',
...
bool relayNotifyDiscord(String body, String& upstreamResponse, int& upstreamCode) {
  // ── 1. Extraire les champs scalaires à la main (pas d'ArduinoJson) ──────
  ...
  // ── 2. Reconvertir oledBufferCompact → oledBuffer (tableau JSON d'entiers)
  //       directement en string, SANS DynamicJsonDocument ───────────────────
  String oledBufferJson = "[]";
  {
    String compactKey = "\"oledBufferCompact\":[";
    int cs = body.indexOf(compactKey);
    if (cs >= 0) {
      cs += compactKey.length();
      int ce = body.indexOf(']', cs);
      if (ce > cs) {
        // Les valeurs sont "00","ff",… → on convertit en entiers décimaux
        String compact = body.substring(cs, ce);
        String out = "[";
        int i = 0;
        bool first = true;
        while (i < (int)compact.length()) {
          // chercher la prochaine paire de guillemets
          ...
          first = false;
          i = q2 + 1;
          yield(); // éviter watchdog sur 1024 itérations
        }
        out += "]";
        oledBufferJson = out;
      }
...
String framesJson = "";
{
  String framesKey = "\"framesCompact\":[";
  int fs = body.indexOf(framesKey);
  if (fs >= 0) {
    fs += framesKey.length();
    int fe = body.indexOf(']', fs); // fin du tableau framesCompact
    // On va reconstruire "frames":[{buffer:[...],delay:N},...]
    // mais c'est trop lourd pour l'ESP → on extrait juste la 1ère frame
    // et on laisse Vercel reconstruire depuis framesCompact
    // → on garde framesCompact tel quel dans le payload final,
    //   Vercel s'occupe de décoder
    framesJson = ""; // Vercel reçoit framesCompact directement
  }
}

  // ── 3. Reconstruire le payload final SANS ArduinoJson ───────────────────
  ...
  // Escape minimal pour les champs texte libres
...
finalPayload += String(",\"forSale\":") + (forSale ? "true" : "false");
  if (text.length()) {
    finalPayload += ",\"text\":\""    + esc(text)      + "\"";
  }
  finalPayload += ",\"oledBuffer\":"  + oledBufferJson;
  // Après la ligne : finalPayload += ",\"oledBuffer\":" + oledBufferJson;
// Ajouter :

// Passer framesCompact tel quel si présent
{
  String fcKey = "\"framesCompact\":";
  int fcp = body.indexOf(fcKey);
  if (fcp >= 0) {
    // Trouver la fin du tableau
    int arrStart = body.indexOf('[', fcp);
    if (arrStart >= 0) {
      ...
      finalPayload += ",\"framesCompact\":";
      finalPayload += body.substring(arrStart, pos + 1);
    }
  }
}
  ...
  Serial.print(F("[ESP] Final payload bytes: "));
  Serial.println(finalPayload.length());
  Serial.print(F("[ESP] Final payload preview: "));
  Serial.println(finalPayload.substring(0, 150));

  // ── 4. Envoi HTTPS vers Vercel ──────────────────────────────────────────
  BearSSL::WiFiClientSecure secureClient;
  secureClient.setInsecure();
  secureClient.setBufferSizes(4096, 512); // ← clé : buffer RX large pour TLS
  ...
  if (!https.begin(secureClient, RESCOE_SYNC_URL)) {
    ...
    upstreamCode = 500;
    Serial.println(F("[ESP] HTTPS begin failed"));
    return false;
  }
