// =====================================================
// ESP8266 + Waveshare 2.7" V2 e-paper
// Test simple avec dessin en RAM + texte + smileys
// =====================================================
/*
// TES branchements
...
// -----------------------------------------------------
// FONT 5x7 ultra simple
// 1 caractère = 5 colonnes, bit0 = haut
// seulement les chars utiles pour le test
// -----------------------------------------------------
...
void setPixel(int x, int y, bool black = true) {
  ...
  uint32_t idx = y * WIDTH_BYTES + (x / 8);
  uint8_t mask = 0x80 >> (x % 8);
  if (black) image[idx] &= ~mask;
  else       image[idx] |= mask;
}

void drawHLine(int x, int y, int w, bool black = true) {
  for (int i = 0; i < w; i++) setPixel(x + i, y, black);
}

void drawVLine(int x, int y, int h, bool black = true) {
  for (int i = 0; i < h; i++) setPixel(x, y + i, black);
}
...
void fillRect(int x, int y, int w, int h, bool black = true) {
  for (int yy = 0; yy < h; yy++) {
    for (int xx = 0; xx < w; xx++) {
      setPixel(x + xx, y + yy, black);
    }
  }
}

void drawCircle(int xc, int yc, int r, bool black = true) {
  int x = 0;
  int y = r;
  int d = 3 - 2 * r;
  while (y >= x) {
...
void fillCircle(int xc, int yc, int r, bool black = true) {
  for (int y = -r; y <= r; y++) {
    for (int x = -r; x <= r; x++) {
      if (x * x + y * y <= r * r) setPixel(xc + x, yc + y, black);
    }
  }
}

void drawChar5x7(int x, int y, char c, int scale = 1, bool black = true) {
  if (c < 32 || c > 90) c = '?';
  uint8_t index = c - 32;
...
void drawText5x7(int x, int y, const char* txt, int scale = 1, bool black = true) {
  while (*txt) {
    if (*txt == '\n') {
      y += 8 * scale;
      x = 8;
    } else {
      drawChar5x7(x, y, *txt, scale, black);
      x += 6 * scale;
    }
    txt++;
  }
...
// VRAIS SMILEYS
// -----------------------------------------------------
void drawHappySmiley(int cx, int cy, int r) {
  Serial.printf("[DRAW] Happy smiley centre=(%d,%d) r=%d\n", cx, cy, r);

  drawCircle(cx, cy, r, true);
  drawCircle(cx, cy, r - 1, true);

  fillCircle(cx - r/3, cy - r/4, 4, true);
  fillCircle(cx + r/3, cy - r/4, 4, true);

  for (int x = -r/2; x <= r/2; x++) {
    int y = (x * x) / (r + 8);
    setPixel(cx + x, cy + r/4 + y, true);
    setPixel(cx + x, cy + r/4 + y + 1, true);
  }
}

void drawSadSmiley(int cx, int cy, int r) {
  Serial.printf("[DRAW] Sad smiley centre=(%d,%d) r=%d\n", cx, cy, r);
  ...
  for (int x = -r/2; x <= r/2; x++) {
    int y = (x * x) / (r + 8);
    setPixel(cx + x, cy + r/2 - y, true);
    setPixel(cx + x, cy + r/2 - y - 1, true);
  }
}

void drawTestPage() {
  Serial.println("[DRAW] clearBuffer()");
  clearBuffer();

  Serial.println("[DRAW] Cadre principal");
  drawRect(0, 0, EPD_W, EPD_H, true);
  drawRect(3, 3, EPD_W - 6, EPD_H - 6, true);

  Serial.println("[DRAW] Titres");
  drawText5x7(12, 12, "EPD TEST", 2, true);
  drawText5x7(12, 34, "ESP8266 OK", 2, true);
  drawHLine(12, 56, 150, true);

  Serial.println("[DRAW] Labels smileys");
  drawText5x7(18, 70, "HAPPY", 2, true);
  drawText5x7(102, 70, "SAD", 2, true);

  drawHappySmiley(45, 125, 28);
  drawSadSmiley(130, 125, 28);

  Serial.println("[DRAW] Zone texte bas");
  drawRect(10, 175, 156, 70, true);
  drawText5x7(18, 186, "TEXT CHECK", 2, true);
  drawText5x7(18, 208, "1234567890", 2, true);
  drawText5x7(18, 230, "ABCXYZ", 2, true);

  Serial.println("[DRAW] Motif pixels");
  for (int i = 0; i < 40; i++) {
    setPixel(120 + i, 200 + (i / 2), true);
    setPixel(120 + i, 235 - (i / 2), true);
  }
  fillRect(118, 198, 4, 4, true);
  fillRect(156, 216, 4, 4, true);
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("======================================");
  Serial.println("BOOT ESP8266 + WAVESHARE 2.7 V2");
  Serial.println("======================================");

  Serial.println("[INFO] Config pins user");
  Serial.printf("[INFO] EPD_SCK  = %d\n", EPD_SCK);
  Serial.printf("[INFO] EPD_MOSI = %d\n", EPD_MOSI);
  Serial.printf("[INFO] EPD_CS   = %d\n", EPD_CS);
  Serial.printf("[INFO] EPD_DC   = %d\n", EPD_DC);
  Serial.printf("[INFO] EPD_RST  = %d\n", EPD_RST);
  Serial.printf("[INFO] EPD_BUSY = %d\n", EPD_BUSY);
  ...
  Serial.println("[INFO] Initialisation EPD...");
  if (epd.Init() != 0) {
    Serial.println("[ERROR] epd.Init() a echoue");
    while (1) {
      Serial.println("[ERROR] STOP");
      delay(1000);
    }
  }
  Serial.println("[OK] epd.Init() OK");

  Serial.println("[STEP] Clear hardware screen");
  epd.Clear();
  Serial.println("[OK] Clear fini");
  delay(1500);

  Serial.println("[STEP] Generation image RAM");
  drawTestPage();
  Serial.printf("[INFO] Buffer size = %d bytes\n", BUFFER_SIZE);
  Serial.println("[OK] Image RAM prete");

  Serial.println("[STEP] Envoi image vers e-paper");
  epd.Display(image);
  Serial.println("[OK] Image envoyee");

  Serial.println("[STEP] Attente BUSY release");
  while (digitalRead(EPD_BUSY) == 1) {
    Serial.println("[BUSY] e-paper occupe...");
    delay(100);
  }
  Serial.println("[DONE] Affichage termine");
  Serial.println("[DONE] Tu dois voir:");
  Serial.println("       - EPD TEST");
  Serial.println("       - ESP8266 OK");
  Serial.println("       - 2 smileys");
  Serial.println("       - TEXT CHECK");
  Serial.println("       - 1234567890");
  Serial.println("       - ABCXYZ");
}
...
void loop() {
  ...
  if (millis() - lastLog > 5000) {
    lastLog = millis();
    Serial.println("[LOOP] programme vivant, ecran conserve son image");
  }
}
...
function buildEinkStillBufferFromOledBuf(buf) {
  ...
  dst.width = EINK_WIDTH; dst.height = EINK_HEIGHT;
  ...
  if (currentCanvasOrientation === 'landscape') {
    // En paysage : on remplit la largeur 264px avec hauteur 176px
    // L'image OLED 128×64 est naturellement en paysage → scale sur 264×(264/2)=132
    const scale = Math.min(EINK_HEIGHT / 128, EINK_WIDTH / 64);
    const w = Math.round(128 * scale);
    const h = Math.round(64  * scale);
    const ox = Math.floor((EINK_HEIGHT - w) / 2);
    const oy = Math.floor((EINK_WIDTH  - h) / 2);
    // Rotation 90° via transform
    dctx.save();
    dctx.translate(EINK_WIDTH / 2, EINK_HEIGHT / 2);
    dctx.rotate(Math.PI / 2);
    dctx.drawImage(src, -w/2, -h/2, w, h);
    dctx.restore();
  } else {
    // Portrait (défaut)
    const scale = Math.min(EINK_WIDTH / 128, EINK_HEIGHT / 64);
    const w  = Math.round(128 * scale);
    const h  = Math.round(64  * scale);
    const ox = Math.floor((EINK_WIDTH  - w) / 2);
    const oy = Math.floor((EINK_HEIGHT - h) / 2);
...
// ── Afficher buffer JS 5808 octets (plein écran) ─────────────────
void renderEinkBuffer(const uint8_t* buf) {
  if (!buf) {
    Serial.println(F("[EINK] buf NULL"));
    return;
  }

  Serial.printf("[EINK] Affichage buffer %d octets\n", EINK_BUF_FULL);

  uint8_t* image = (uint8_t*)malloc(EINK_BUF_FULL);
  if (!image) {
    Serial.println(F("[EINK] malloc FAILED"));
    return;
  }

  // Inversion pour Waveshare : JS noir=1 → Waveshare noir=0
  for (int i = 0; i < EINK_BUF_FULL; i++) {
    image[i] = ~buf[i];
  }
  ...
  // FONCTION QUI EXISTE VRAIMENT dans ton .cpp !
  epd.Display(image);
...
if (req.startsWith("GET /eink-username?")) {
  ...
  pendingEinkAuthor = getQueryParam(req, "n").substring(0, 20);
  pendingEinkTimestamp = getQueryParam(req, "ts").substring(0, 23);
  pendingEinkArtistName = getQueryParam(req, "a").substring(0, 20);
  pendingEinkEthAddress = getQueryParam(req, "eth").substring(0, 42);
...
if (req.startsWith("POST /eink-draw")) {
  ...
  // Refresh e-ink après réponse HTTP
